    ...
    if message["type"] != "token" and message["from_player"] != MY_ID and message["to_player"] == MY_ID:
        if message["type"] == "receive_guesses":
            ...
            if message["from_player"] != (MY_ID+3)%4:
                ...
                move = make_move()
                MOVES[MY_ID] = move
                rin(f"[DEBUG] MOVES da rodada: {MOVES}")
                count_points()
        ...
        if message['type'] == "inform_guesses":
            ...
            a = input("Acho que nao está mais usando esse aqui, se vc estiver sendo esse print PFV informe ao JP (nao deveria estar mais vindo pra esse estado nao viu... aiai")
            # end_cicle_info = ()
            # for i in range(1, 4):
            #     msg = {
            #         "type": "end_cicle_info",
